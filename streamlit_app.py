import streamlit as st import datetime import random from io import BytesIO import json import base64 import pandas as pd
...
if "logs" not in st.session_state: st.session_state.logs = [] if "scripts" not in st.session_state: st.session_state.scripts = {"Little Ummahs": "", "Sunnah Mindset": ""} if "prompts" not in st.session_state: st.session_state.prompts = {"Little Ummahs": [], "Sunnah Mindset": []} if "current_view" not in st.session_state: st.session_state.current_view = "Dashboard Overview"

=== HEADER ===

st.markdown(""" <h1 style='text-align: center; font-size: 42px;'>🕌 Halal Control Panel</h1> <p style='text-align: center;'>Welcome, <strong>Puchu</strong> 👋</p> """, unsafe_allow_html=True)

=== NAVIGATION TOGGLE ===

view_options = ["Dashboard Overview", "Sunnah Mindset", "Little Ummahs"] view = st.selectbox("Select View", view_options, index=view_options.index(st.session_state.current_view)) st.session_state.current_view = view

=== VIEW ROUTING ===

if view == "Dashboard Overview": st.markdown("### 📊 Dashboard Overview") st.info("Welcome to the central dashboard. Use the toggle above to switch views.") st.markdown("### 📜 Activity Logs") if st.session_state.logs: for log in reversed(st.session_state.logs): st.markdown(f"{log['time']} — {log['channel']} Video → {log['status']} 🔗 Link") else: st.info("No logs yet. Start a task to see results.")
...
elif view in ["Little Ummahs", "Sunnah Mindset"]: chan = view key = chan.replace(" ", "_").lower()
